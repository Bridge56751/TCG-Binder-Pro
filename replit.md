...
- **In-App Purchases**: RevenueCat SDK (`react-native-purchases`) for iOS in-app purchases. `PurchaseContext` wraps the app and manages premium status. Entitlement ID: `TCG Binder Pro Unlimited`. Public API key via `EXPO_PUBLIC_REVENUECAT_API_KEY` env var. Backend verification via `REVENUECAT_SECRET_API_KEY`. Works in Expo Go via Preview API Mode.
