...
- **Meta (Facebook) SDK**: `react-native-fbsdk-next` for install attribution and ad campaign tracking. Facebook App ID: `1622411945454184`, Client Token: `8ff8bb45dd6e1caf72a2dce249c46834`. Configured in app.json plugins with advertiser ID collection, auto event logging, and auto-init enabled. `expo-tracking-transparency` requests iOS ATT permission on launch (in `_layout.tsx`). FB anonymous ID passed to RevenueCat via `setFBAnonymousID` in `PurchaseContext.tsx` for subscription attribution. Both SDKs conditionally loaded (native only) to avoid web crashes.
