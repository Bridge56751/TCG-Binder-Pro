...
- **Local storage**: `@react-native-async-storage/async-storage` stores the user's card collection as a JSON structure keyed by game → set → card IDs. Cloud sync available for logged-in users via `/api/collection/sync`. Collection mutations (add/remove/clear) are serialized via a promise queue in `collection-storage.ts` to prevent race conditions. An `AppState` listener in `CollectionContext.tsx` reads the latest data from AsyncStorage (authoritative source) and syncs to cloud when the app goes to background. Batch "Add All" triggers an immediate sync after completion.
- **Sync data loss protection**: Server-side guard rejects syncs where incoming card count drops below 50% of stored count (for collections >10 cards), returning 409. The updated client auto-retries via `/api/collection/sync/force`. The old client will simply fail the sync (preserving local data). Previous collection state is backed up in `previousData` column on every sync. `/api/collection/restore-backup` endpoint allows restoring the previous state.
...
- **Tracking Transparency**: `expo-tracking-transparency` requests iOS ATT permission on launch (in `_layout.tsx`). Facebook SDK (`react-native-fbsdk-next`) was removed due to Xcode build incompatibility; attribution tracking relies on RevenueCat's built-in attribution.
...
- **Key routes**:
  ...
  - `/api/collection/sync` — GET/POST collection sync for logged-in users (POST has data loss guard)
  - `/api/collection/sync/force` — POST force sync bypassing data loss guard
  - `/api/collection/restore-backup` — POST restore previous collection from backup
...
- **Current schema**: Users table (id, email, password, appleId, rcSubscriberId, isPremium, isVerified, verificationCode, verificationExpiry, resetCode, resetExpiry) and user_collections table (userId FK, data JSONB, previousData JSONB, updatedAt) and chat-related tables (conversations, messages). The TCG card/set data comes from external APIs, not the database.
